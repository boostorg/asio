...
#include <boost/asio/use_await.hpp>
#include <list>
#include <iostream>

using boost::asio::ip::tcp;
using boost::asio::use_await;
...
simple_coro listener(tcp::acceptor& acceptor)
{
  for (;;)
  {
    if (auto [_, socket] = co_await acceptor.async_accept(use_await); socket.is_open())
    {
      ...
      {
        char data[1024];
        auto [e1, n] = co_await socket.async_read_some(boost::asio::buffer(data), use_await);
        if (e1) break;
        auto [e2, _] = co_await boost::asio::async_write(socket, boost::asio::buffer(data, n), use_await);
        if (e2) break;
      }
    }
  }
...
simple_coro stopper(boost::asio::io_context& ctx)
{
